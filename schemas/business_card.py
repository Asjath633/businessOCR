...
class BusinessCardData(BaseModel):
    ...
    name: Optional[List[str]] = Field(default_factory=list, description="Full name(s) of the person(s) on the card")
    organization: Optional[str] = Field(None, description="Company or organization name")
    designation: Optional[str] = Field(None, description="Job title or designation")
    email: Optional[List[str]] = Field(default_factory=list, description="Email address(es)")
    phone: Optional[List[str]] = Field(default_factory=list, description="Phone numbers")
...
class OCRResult(BaseModel):
    ...
    confidence: Optional[float] = Field(None, description="Overall OCR confidence score")
